...
module.exports = async function (context, req) {
    ...
    try {
        ...
        if (stripeSignature && endpointSecret) {
            ...
            // This block is for your old mock events, kept for compatibility
            event = { type: req.body.event_type, data: { object: req.body } };
            context.log('Processing mock event:', event.type);
        }

        // Handle the event
        switch (event.type) {
            // NEW HANDLER FOR THE FREE TRIAL CREATION
        ...
        context.res = { status: 200, body: { received: true, event_type: event.type }};

    } catch (error) {
        context.log.error('Error processing webhook:', error);
        context.res = { status: 500, body: { message: error.message || 'Internal server error' } };
    }
};

// ===================================================================
//  NEW FUNCTION TO HANDLE TRIAL CREATION
// ===================================================================
async function handleSubscriptionCreated(context, subscription) {
    // This event fires for both new trials and new paid subscriptions.
    // We only want to act on the initial creation of a trial.
    if (subscription.status !== 'trialing') {
        context.log(`Subscription ${subscription.id} is not a trial. Status: ${subscription.status}. Ignoring.`);
        return;
    }

    try {
        context.log(`Processing trial subscription creation for sub: ${subscription.id}`);
        
        const customer = await stripe.customers.retrieve(subscription.customer);
        const email = customer.email;
        const licenseCount = subscription.items.data.reduce((total, item) => total + item.quantity, 0);

        // Idempotency Check: Make sure we haven't already created this organization
        const orgQuery = {
            query: "SELECT * FROM c WHERE c.stripeCustomerId = @customerId",
            parameters: [{ name: "@customerId", value: customer.id }]
        };
        const { resources: existingOrgs } = await organizationsContainer.items.query(orgQuery).fetchAll();
        if (existingOrgs.length > 0) {
            context.log(`Organization for customer ${customer.id} already exists. Skipping creation.`);
            return;
        }

        // --- Create New Organization ---
        const organizationId = uuidv4();
        const companyName = customer.metadata.companyName || `${email.split('@')[0]}'s Company`;
        ...
        const organization = {
            ...
            status: 'trialing', // Set status to 'trialing'
            adminEmail: email,
        ...
        context.log(`Created new organization in trial: ${organizationId}`);

        // --- Create New User (Admin) ---
        const userId = uuidv4();
        const firstName = customer.name?.split(' ')[0] || 'Admin';
        const lastName = customer.name?.split(' ').slice(1).join(' ') || 'User';
        ...
        context.log(`Created new admin user for trial: ${email}`);

    } catch (error) {
        context.log.error('Error in handleSubscriptionCreated:', error);
        throw error;
    }
}

// ===================================================================
//  YOUR EXISTING HANDLER FUNCTIONS (UNCHANGED)
// ===================================================================
...
async function handleSubscriptionUpdated(context, subscription) {
    try {
        ...
        const updatedOrg = { ...organization, status: newStatus, lastUpdated: new Date().toISOString() };
...
async function handleSubscriptionDeleted(context, subscription) {
    try {
        ...
        const organization = organizations[0];
        const updatedOrg = { ...organization, status: 'cancelled', cancelledAt: new Date().toISOString(), lastUpdated: new Date().toISOString() };
        await organizationsContainer.item(organization.id, organization.id).replace(updatedOrg);
...
async function handlePaymentFailed(context, invoice) {
    try {
        ...
        const organization = organizations[0];
        const updatedOrg = { ...organization, status: 'past_due', gracePeriodEnd: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), lastUpdated: new Date().toISOString() };
        await organizationsContainer.item(organization.id, organization.id).replace(updatedOrg);
...
async function handlePaymentSucceeded(context, invoice) {
    try {
        ...
        const organization = organizations[0];
        const updatedOrg = { ...organization, status: 'active', lastUpdated: new Date().toISOString() };
        delete updatedOrg.gracePeriodEnd;
        await organizationsContainer.item(organization.id, organization.id).replace(updatedOrg);
...
async function handleMockSubscriptionUpdated(context, email, status = 'active') {
    try {
        ...
        const organization = organizations[0];
        const updatedOrg = { ...organization, status: status, lastUpdated: new Date().toISOString() };
        if (status === 'active') {
